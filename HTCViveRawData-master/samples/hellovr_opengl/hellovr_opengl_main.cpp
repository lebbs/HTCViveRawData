        ...
        if (m_pHMD->GetControllerState(unDevice, &state, sizeof(state)))
        {
            ...
            case vr::ETrackedDeviceClass::TrackedDeviceClass_Controller:
                ...
                case vr::TrackedControllerRole_RightHand:
					...
					//KIRJOITTAA DATAN TIEDOSTOON
					ofstream MyFile;
					MyFile.open("Testi12345.csv", std::ios_base::app);

					const auto p1 = std::chrono::system_clock::now();
					
					LARGE_INTEGER qpc1, qpc2,fr;
					QueryPerformanceCounter(&qpc1);
					auto current_time = std::chrono::system_clock::now();


					MyFile << current_time.time_since_epoch().count() << "," << position.v[0] << "," << position.v[1] << "," << position.v[2] <<"\n";
					...
					/*MyFile << "X = " << position.v[0] << " , " << "Y = " << position.v[1] << " , " << "Z = " << position.v[2] << " , "
						<< "QW = " << quaternion.w << " , " << "QX = " << quaternion.x << " , " << "QY = " << quaternion.y << " , "
						<< "QZ = " << quaternion.z << "\n";*/
					...
					//"x = %.5f, y = %.5f, z = %.5f, qw = %.5f, qx = %.5f, qy = %.5f, qz = %.5f",
